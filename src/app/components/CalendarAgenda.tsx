...
function fromApi(e: ApiCalendarEvent): SessionEvent {
  return {
    id: e.id,
    title: e.title,
    ...
    type: e.type,
    players: e.players || [],
    maxPlayers: e.maxPlayers ?? e.max_players ?? 5,
    isRecurring: e.isRecurring ?? e.is_recurring ?? false,
    recurringType: (e.recurringType || e.recurring_type) as SessionEvent["recurringType"],
    status: e.status,
    reminder: e.reminder,
...
export function CalendarAgenda({ userType, userId }: CalendarAgendaProps) {
  ...
  useEffect(() => {
    if (!userId) return;
    calendarApi.list(userId)
      .then(list => setEvents(list.map(fromApi)))
      .catch(err => {
        console.warn("[crytto] falha ao carregar eventos:", err);
  ...
  const createEvent = async () => {
    try {
      const dateStr = (formData.date instanceof Date ? formData.date : new Date())
  ...
  const updateEvent = async () => {
    ...
    try {
      ...
      if (formData.date instanceof Date) {
        patch.date = formData.date.toISOString().slice(0, 10);
      }
      const updated = await calendarApi.update(selectedEvent.id, patch);
      const merged = fromApi(updated);
