...
import { charactersApi, type ApiCharacter } from "../lib/api";
import {
  listCharacters,
  upsertCharacter,
  removeCharacter as removeStoredCharacter,
  uuid,
...
function fromApi(c: ApiCharacter): Character {
  return {
    id: c.id,
    name: c.name,
    class: c.class,
...
export function CharacterManager({ userType, userId }: CharacterManagerProps) {
  const [characters, setCharacters] = useState<Character[]>([]);

  useEffect(() => {
    if (!userId) return;
    // Fonte de verdade: localStorage — garante persistência após refresh sem depender do backend.
    const local = listCharacters(userId);
    setCharacters(local);

    // Sincronização best-effort com o backend só quando não há nada salvo localmente.
    if (local.length === 0) {
      charactersApi.list(userId)
        .then(list => {
          if (list && list.length) {
            list.map(fromApi).forEach(c => upsertCharacter(userId, c));
            setCharacters(listCharacters(userId));
          }
        })
        .catch(() => { /* backend indisponível: seguimos apenas com dados locais */ });
    }
  }, [userId]);
  ...
  const createCharacter = async () => {
    const newCharacter: Character = {
      ...
      level: 1, // nível fixo: o campo "adicionar nível" foi removido dos requisitos
    ...
    // Persiste localmente (fonte de verdade) e reflete na UI imediatamente.
    setCharacters(upsertCharacter(userId, newCharacter));
    resetFormData();
    setIsCreateDialogOpen(false);
    toast.success(`✨ Personagem ${newCharacter.name} criado com sucesso!`);

    // Sincronização best-effort com o backend.
    charactersApi.create({
      ...
      avatar_url: newCharacter.avatarUrl,
      hp: newCharacter.hp,
      ac: newCharacter.ac,
  ...
  const updateCharacter = async () => {
    ...
    const updated: Character = {
      ...
      level: selectedCharacter.level, // mantém o nível existente (sem edição de nível)
    } as Character;

    setCharacters(upsertCharacter(userId, updated));
    resetFormData();
    setIsEditDialogOpen(false);
    setSelectedCharacter(null);
    toast.success(`✅ Personagem atualizado com sucesso!`);

    charactersApi.update(updated.id, {
      name: updated.name,
      class: updated.class,
      level: updated.level,
      race: updated.race,
  ...
  const deleteCharacter = async (id: string) => {
    setCharacters(removeStoredCharacter(userId, id));
    toast.success("🗑️ Personagem removido!");
    charactersApi.remove(id).catch(() => { /* backend indisponível */ });
  };

  // Render helper (NÃO é um componente): renderizado inline para evitar que os
  // inputs percam o foco a cada tecla. Definir um componente aqui e usá-lo como
  // <CharacterForm /> fazia o React remontar a subárvore a cada render.
  const renderCharacterForm = (isEdit = false) => (
